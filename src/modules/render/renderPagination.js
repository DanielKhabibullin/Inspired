...
export const renderPagination = (wrapperPagination, page, pages, count) => {
	...
	if (pages > count) {
		createElement('a',
			{
				className: `pagination__arrow  pagination__arrow-start  ${isNotStart ? 'pagination__arrow_disabled': ''}`,
				href: !isNotStart ? `${router.getCurrentLocation().url}?page=${1}` : '',
				textContent: 'start',
				ariaLabel: 'В начало',
			},
			{
		...
		createElement('a',
			{
				className: `pagination__arrow  pagination__arrow-end  ${isEnd ? 'pagination__arrow_disabled': ''}`,
				href: isEnd ? '' : `${router.getCurrentLocation().url}?page=${pages}`,
				textContent: 'end',
				ariaLabel: 'В конец',
			},
			{
